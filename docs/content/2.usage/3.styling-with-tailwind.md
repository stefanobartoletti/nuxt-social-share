...
`<SocialShare>` hooks into this setup, allowing it to override basic resets from the `base` layer while still being further customizable by utilities in the `utilities` layer.
...
Unfortunately, the different internal setup of Tailwind v3 and v4 made it impossible to keep compatibility with both versions at the same time. When updating the Nuxt Social Share module, the current v4 version has been prioritized, discarding support for the outdated v3.
